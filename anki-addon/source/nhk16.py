        ...
        for entry in entries:
            ...
            for accent in entry["accents"]:
                ...
                if len(expression_list) == 0:
                    insert_entry(
                        (reading, reading, self.data.id, display_text, sound_file)
                    )  # entry
                for expression in expression_list:
                    insert_entry(
                        (expression, reading, self.data.id, display_text, sound_file)
                    )  # entry
                ...
                if "head" in subentry:
                    ...
                    for accent in subentry["accents"]:
                        ...
                        for head in head_list:
                            if is_kana(head):
                                insert_entry(
                                    (head, head, self.data.id, display_text, sound_file)
                                )  # subentry
                            else:
                                insert_entry(
                                    (head, None, self.data.id, display_text, sound_file)
                                )  # subentry
